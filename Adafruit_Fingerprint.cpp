...
Adafruit_Fingerprint::Adafruit_Fingerprint(unsigned long serial, uint32_t password) {
...
void Adafruit_Fingerprint::begin() {
  MAP_UtilsDelay(1000000); // one second delay to let the sensor 'boot up'

  MAP_UARTConfigSetExpClk(UARTA1_BASE, MAP_PRCMPeripheralClockGet(PRCM_UARTA1),
                  57600, (UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE |
...
void Adafruit_Fingerprint::writeStructuredPacket(
    ...
    MAP_UARTCharPut(mySerial, (uint8_t)(packet.start_code >> 8));
    MAP_UARTCharPut(mySerial, (uint8_t)(packet.start_code & 0xFF));
    MAP_UARTCharPut(mySerial, (packet.address[0]));
    MAP_UARTCharPut(mySerial, (packet.address[1]));
    MAP_UARTCharPut(mySerial, (packet.address[2]));
    MAP_UARTCharPut(mySerial, packet.address[3]);
    MAP_UARTCharPut(mySerial, packet.type);

  uint16_t wire_length = packet.length + 2;
  MAP_UARTCharPut(mySerial, (uint8_t)(wire_length >> 8));
  MAP_UARTCharPut(mySerial, (uint8_t)(wire_length & 0xFF));

#ifdef FINGERPRINT_DEBUG
  Report("-> 0x");
  Report("%x\n\r", (uint8_t)(packet.start_code >> 8));
  Report(", 0x");
  Report("%x\n\r", (uint8_t)(packet.start_code & 0xFF));
  Report(", 0x");
  Report("%x\n\r", packet.address[0]);
  Report(", 0x");
  Report("%x\n\r", packet.address[1]);
  ...
  Report("%x\n\r", packet.type);
  Report(", 0x");
  Report("%x\n\r", (uint8_t)(wire_length >> 8));
  Report(", 0x");
  Report("%x\n\r", (uint8_t)(wire_length & 0xFF));
#endif

  uint16_t sum = ((wire_length) >> 8) + ((wire_length)&0xFF) + packet.type;
  for (uint8_t i = 0; i < packet.length; i++) {
      MAP_UARTCharPut(mySerial, packet.data[i]);
    sum += packet.data[i];
#ifdef FINGERPRINT_DEBUG
    Report(", 0x");
    Report("%x\n\r", packet.data[i]);
  ...
  MAP_UARTCharPut(mySerial, (uint8_t)(sum >> 8));
  MAP_UARTCharPut(mySerial, (uint8_t)(sum & 0xFF));
...
Adafruit_Fingerprint::getStructuredPacket(Adafruit_Fingerprint_Packet *packet,
  ...
  while (true) {
    while (!MAP_UARTCharsAvail(mySerial)) {
      MAP_UtilsDelay(1000);
      timer++;
      if (timer >= timeout) {
#ifdef FINGERPRINT_DEBUG
